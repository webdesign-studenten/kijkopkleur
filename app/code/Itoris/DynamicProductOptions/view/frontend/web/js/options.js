...
window.DynamicProductOptions = {
    ...
    initialize : function(config, options, isGrouped, tierPrices, translations, taxInfo) {
        ...
        this.priceFormat = this.getPriceBox().data('magePriceBox').options.priceConfig.priceFormat;
    ...
    prepareOptions : function() {
        ...
        this.getPriceBox().on('updatePrice', function(ev, prices){
            ...
            for(key in prices) {
                ...
                if (config.type == 'drop_down') {_key +='_'+config.elm.value; config = obj.optionConfigById[_key];}
            ...
            for (var j = 0; j < allRadios.length; j++) {
                if (allRadios[j].value == option.items[i].option_type_id) {
                    ...
                    if (option.items[i].swatch && option.items[i].swatchhtml) {
                        ...
                        option.items[i].swatchhtml = option.items[i].swatchhtml.replace(/{price}/g, formatCurrency(option.items[i].price ? parseFloat(option.items[i].price) : 0, this.priceFormat, false));
