...
class UpgradeSchema implements UpgradeSchemaInterface
{
    ...
    public function upgrade(SchemaSetupInterface $setup, ModuleContextInterface $context)
    {
        ...
        if (version_compare($context->getVersion(), '2.7.3') < 0) {
            $setup->run("ALTER TABLE `{$setup->getTable('itoris_dynamicproductoptions_template')}` ADD `sku_order` TEXT NOT NULL AFTER `absolute_sku`;");
            
            $setup->run("ALTER TABLE `{$setup->getTable('itoris_dynamicproductoptions_options')}` ADD `sku_order` TEXT NOT NULL AFTER `absolute_sku`;");
        }
